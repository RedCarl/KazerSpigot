...
public class PandaRedstoneWire extends BlockRedstoneWire {
	...
	/**
	 * Positions of wire that was updated already (Ordering determines update order
	 * and is therefore required!)
	 **/
	private final Set<BlockPosition> updatedRedstoneWire = Sets.newLinkedHashSet();

	/**
	 * Ordered arrays of the facings; Needed for the update order. I went with a
	 * vertical-first order here, but vertical last would work to. However it should
	...
	private void updateSurroundingRedstone(World worldIn, BlockPosition pos, IBlockData iblockdata) {
		...
		/*
		 * Avoid unnecessary updates on the just updated wires A huge scale test showed
		 * about 40% more ticks per second It's probably less in normal usage but likely
		 * still worth it
		 */
	...
	/**
	 * Checks if an wire needs to be marked for update depending on the power next
	 * to it
	 *
	 * @author panda
	 *
	 ...
	 * Checks all wires below before the same layer before on top to keep some more
	...
	protected void checkSurroundingWires(World worldIn, BlockPosition pos) {
		...
		for (EnumDirection facingVertical : facingsVertical) {
			...
			for (EnumDirection facingHorizontal : facingsHorizontal) {
				...
				// doesn't cut off the wire (is non-solid)
	...
	/**
	 * Adds all blocks that need to receive an update from a redstone change in this
	 * position. This means only blocks that actually could change.
	 *
	 * @author panda
	 *
	 * @param worldIn World
	...
	/**
	 * Checks if a block can get powered from a side. This behavior would better be
	 * implemented per block type as follows: - return false as default. (blocks
	 * that are not affected by redstone don't need to be updated, it doesn't really
	 * hurt if they are either) - return true for all blocks that can get powered
	 * from all side and change based on it (doors, fence gates, trap doors, note
	 ...
	 * one of the same line. This is to prefer as it makes understanding the update
	 * order of the wire really easy. The signal "travels" from the power source.
	 *
	 * @author panda
	 *
	 * @param state  State of the block
	 * @param side   Side from which it gets powered
	 * @param isWire True if it's powered by a wire directly, False if through a
	 *               block
	...
	/**
	 * Creates a list of all horizontal sides that can get powered by a wire. The
	 * list is ordered the same as the facingsHorizontal.
	 *
	 * @param worldIn World
	 * @param pos     Position of the wire
