...
public class ServerConnection {
	...
	public static final LazyInitVar<DefaultEventLoopGroup> c = new LazyInitVar<>(() -> new DefaultEventLoopGroup(0,
		...
		synchronized (this.getListeningChannels()) {
			...
			this.getListeningChannels()
					.add(((new ServerBootstrap().channel(channel))
							...
							.childHandler(new MinecraftPipeline(this)).group(a.get(), b.get()).localAddress(ip, port))
				...
				if (!networkmanager.h()) {
					if (!networkmanager.isConnected()) {
						...
						try {
							...
							ServerConnection.LOGGER.warn(
									"Failed to handle packet for " + networkmanager.getSocketAddress(), exception);
