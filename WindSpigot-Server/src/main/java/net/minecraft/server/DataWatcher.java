...
//        this.f.writeLock().unlock(); // Spigot - not required
	...
	private DataWatcher.WatchableObject j(int i) {
		 ...
		 * 
		 * try { datawatcher_watchableobject = (DataWatcher.WatchableObject)
		 * this.dataValues.get(i); // Spigot } catch (Throwable throwable) { CrashReport
		 * crashreport = CrashReport.a(throwable, "Getting synched entity data");
		 * CrashReportSystemDetails crashreportsystemdetails =
		 * crashreport.a("Synched entity data");
		 * 
		 * crashreportsystemdetails.a("Data ID", (Object) Integer.valueOf(i)); throw new
		 * ReportedException(crashreport); }
		 * 
		 * this.f.readLock().unlock(); return datawatcher_watchableobject;
		 * 
		 */
