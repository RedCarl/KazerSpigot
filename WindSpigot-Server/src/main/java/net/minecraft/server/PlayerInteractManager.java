...
public class PlayerInteractManager {
	...
	public void a(BlockPosition blockposition, EnumDirection enumdirection) {
		// CraftBukkit start
		PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(this.player, Action.LEFT_CLICK_BLOCK,
		...
		if (this.isCreative()) {
			...
			org.bukkit.event.block.BlockDamageEvent blockEvent = CraftEventFactory.callBlockDamageEvent(this.player,
					blockposition.getX(), blockposition.getY(), blockposition.getZ(),
					this.player.inventory.getItemInHand(), f >= 1.0f);
	...
	public boolean breakBlock(BlockPosition blockposition) {
		...
		if (this.player instanceof EntityPlayer) {
			...
			boolean isSwordNoBreak = this.gamemode.d() && this.player.bA() != null
					&& this.player.bA().getItem() instanceof ItemSword;
	...
	public boolean interact(EntityHuman entityhuman, World world, ItemStack itemstack, BlockPosition blockposition,
			EnumDirection enumdirection, float f, float f1, float f2) {
		/*
		 * CraftBukkit start - whole method if (this.gamemode ==
		 * WorldSettings.EnumGamemode.SPECTATOR) { TileEntity tileentity =
		 * world.getTileEntity(blockposition);
		 * 
		 * if (tileentity instanceof ITileInventory) { Block block =
		 * world.getType(blockposition).getBlock(); ITileInventory itileinventory =
		 * (ITileInventory) tileentity;
		 * 
		 * if (itileinventory instanceof TileEntityChest && block instanceof BlockChest)
		 * { itileinventory = ((BlockChest) block).f(world, blockposition); }
		 * 
		 * if (itileinventory != null) { entityhuman.openContainer(itileinventory);
		 * return true; } } else if (tileentity instanceof IInventory) {
		 * entityhuman.openContainer((IInventory) tileentity); return true; }
		 * 
		 * return false; } else { if (!entityhuman.isSneaking() || entityhuman.bA() ==
		 * null) { IBlockData iblockdata = world.getType(blockposition);
		 * 
		 * if (iblockdata.getBlock().interact(world, blockposition, iblockdata,
		 * entityhuman, enumdirection, f, f1, f2)) { return true; } }
		 * 
		 * if (itemstack == null) { return false; } else if (this.isCreative()) { int i
		 * = itemstack.getData(); int j = itemstack.count; boolean flag =
		 * itemstack.placeItem(entityhuman, world, blockposition, enumdirection, f, f1,
		 * f2);
		 * 
		 * itemstack.setData(i); itemstack.count = j; return flag; } else { return
		 * itemstack.placeItem(entityhuman, world, blockposition, enumdirection, f, f1,
		 * f2); } } // Interract event
		 */
		...
		if (blockdata.getBlock() != Blocks.AIR) {
			...
			PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(entityhuman, Action.RIGHT_CLICK_BLOCK,
					blockposition, enumdirection, itemstack, cancelledBlock);
			firedInteract = true;
			...
			if (event.useInteractedBlock() == Event.Result.DENY) {
				...
				if (blockdata.getBlock() instanceof BlockDoor) {
					...
					((EntityPlayer) entityhuman).playerConnection.sendPacket(
							new PacketPlayOutBlockChange(world, bottom ? blockposition.up() : blockposition.down()));
				}
				...
				result = blockdata.getBlock().interact(world, blockposition, blockdata, entityhuman, enumdirection, f,
						f1, f2);
			}
