...
//						// WindSpigot end
	...
	public void g(float f, float f1) {
		...
		if (this.abilities.isFlying && this.vehicle == null) {
			...
			this.aM = this.abilities.a() * (this.isSprinting() ? 2 : 1);
			super.g(f, f1);
			this.motY = d3 * 0.6D;
			this.aM = f2;
		} else {
		...
		if (this.vehicle == null) {
			...
			if (this.a(Material.WATER)) {
				...
				if (i > 0) {
					...
					this.applyExhaustion(world.paperSpigotConfig.playerSwimmingExhaustion * i * 0.01F); // PaperSpigot
																										// -
																										// Configurable
																										// swimming
																										// exhaustion
				...
				if (i > 0) {
					...
					if (this.isSprinting()) {
						...
						this.applyExhaustion(0.099999994F * i * 0.01F);
					} else {
						if (this.isSneaking()) {
							this.a(StatisticList.j, i);
						}

						this.applyExhaustion(0.01F * i * 0.01F);
					}
				}
			} else {
				i = Math.round(MathHelper.sqrt(d0 * d0 + d2 * d2) * 100.0F);
	...
	public void e(float f, float f1) {
		if (!this.abilities.canFly) {
			if (f >= 2.0F) {
				this.a(StatisticList.m, (int) Math.round(f * 100.0D));
			}
