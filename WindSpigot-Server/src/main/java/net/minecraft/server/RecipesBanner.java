...
public class RecipesBanner {
	...
	static class AddRecipe extends ShapelessRecipes implements IRecipe { // CraftBukkit - added extends
		...
		private TileEntityBanner.EnumBannerPatternType c(InventoryCrafting inventorycrafting) {
			TileEntityBanner.EnumBannerPatternType[] atileentitybanner_enumbannerpatterntype = TileEntityBanner.EnumBannerPatternType
					.values();
			...
			for (TileEntityBanner.EnumBannerPatternType tileentitybanner_enumbannerpatterntype : atileentitybanner_enumbannerpatterntype) {
				if (tileentitybanner_enumbannerpatterntype.d()) {
						...
						for (k = 0; k < inventorycrafting.getSize() && flag; ++k) {
							...
							if (itemstack != null && itemstack.getItem() != Items.BANNER) {
								if (itemstack.getItem() == Items.DYE) {
									...
									if (flag1 || !itemstack
											.doMaterialsMatch(tileentitybanner_enumbannerpatterntype.f())) {
										flag = false;
										break;
									}
					...
					} else if (inventorycrafting.getSize() != tileentitybanner_enumbannerpatterntype.c().length
							* tileentitybanner_enumbannerpatterntype.c()[0].length()) {
						flag = false;
					} else {
						int l = -1;
