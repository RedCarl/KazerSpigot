	...
	public void t_() {
		...
		if (this.az > 0) {
			...
			if (movingobjectposition != null) {
				...
				this.getBukkitEntity().setVelocity(this.getBukkitEntity().getVelocity().multiply(this.owner.getKnockback().getRodSpeed()));
					...
					if (this.owner != null) {
						if (movingobjectposition.entity instanceof EntityPlayer) {
							...
							if (((CraftPlayer)this.owner.getBukkitEntity()).canSee(hit.getBukkitEntity()) && movingobjectposition.entity.damageEntity(DamageSource.projectile(this, this.owner), 0.0f)) {
								...
								double velX = v.getX() / 1.6 * profile.getRodHorizontal();
								double velY = 0.36 * profile.getRodVertical();
								double velZ = v.getZ() / 1.6 * profile.getRodHorizontal();
		...
		if (this.world.isClientSide) {
			...
			if (this.hooked != null) {
				...
				PlayerFishEvent playerFishEvent = new PlayerFishEvent((Player) this.owner.getBukkitEntity(),
						this.hooked.getBukkitEntity(), (Fish) this.getBukkitEntity(),
