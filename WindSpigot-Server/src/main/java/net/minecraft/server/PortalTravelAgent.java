...
public class PortalTravelAgent {
	...
	private final ConcurrentLinkedQueue<Long> d = new ConcurrentLinkedQueue<>(); // Nacho :: ArrayList ->
		...
		for (int l = -2; l <= 2; ++l) {
			for (int i1 = -2; i1 <= 2; ++i1) {
				for (int j1 = -1; j1 < 3; ++j1) {
					...
					this.a.setTypeUpdate(new BlockPosition(k1, l1, i2),
			...
			for (int i1 = -2; i1 <= 2; ++i1) {
				for (int j1 = -1; j1 < 3; ++j1) {
					...
					if (this.a.getType(new BlockPosition(k1, l1, i2))
							.getBlock() != (flag ? Blocks.OBSIDIAN : Blocks.AIR)) {
						return null;
					}
				}
			}
	...
	public boolean b(Entity entity, float f) {
		...
		// BlockPosition found = this.findPortal(entity.locX, entity.locY, entity.locZ,
		// 128);
		// MinetickMod start
	...
	public BlockPosition findPortal(double x, double y, double z, int searchRadius) { // Paper - actually use search
																						// radius
		...
		if (this.c.contains(k)) {
			...
			for (int l = -searchRadius; l <= searchRadius; ++l) { // Paper - actually use search radius
				...
				for (int i1 = -searchRadius; i1 <= searchRadius; ++i1) { // Paper - actually use search radius
					for (BlockPosition blockposition2 = blockposition.a(l, this.a.V() - 1 - blockposition.getY(),
	...
	public void adjustExit(Entity entity, Location position, Vector velocity) {
		...
		if (this.a.getWorld().getEnvironment() == org.bukkit.World.Environment.THE_END
				|| entity.getBukkitEntity().getWorld().getEnvironment() == org.bukkit.World.Environment.THE_END
				|| entity.aG() == null) {
			...
			ShapeDetector.ShapeDetectorCollection shapedetector_shapedetectorcollection = Blocks.PORTAL.f(this.a,
					object);
			boolean flag2 = shapedetector_shapedetectorcollection.b().e()
					.c() == EnumDirection.EnumAxisDirection.NEGATIVE;
			double d5 = shapedetector_shapedetectorcollection.b().k() == EnumDirection.EnumAxis.X
					? (double) shapedetector_shapedetectorcollection.a().getZ()
					: (double) shapedetector_shapedetectorcollection.a().getX();

			d3 = shapedetector_shapedetectorcollection.a().getY() + 1
					- entity.aG().b * shapedetector_shapedetectorcollection.e();
			if (flag2) {
				++d5;
			}

			if (shapedetector_shapedetectorcollection.b().k() == EnumDirection.EnumAxis.X) {
				d4 = d5 + (1.0D - entity.aG().a) * shapedetector_shapedetectorcollection.d()
						* shapedetector_shapedetectorcollection.b().e().c().a();
			} else {
				d2 = d5 + (1.0D - entity.aG().a) * shapedetector_shapedetectorcollection.d()
						* shapedetector_shapedetectorcollection.b().e().c().a();
			}
	...
	public boolean createPortal(double x, double y, double z, int b0) {
		...
		for (i2 = i - b0; i2 <= i + b0; ++i2) {
			...
			for (j2 = k - b0; j2 <= k + b0; ++j2) {
				...
				label271: for (k2 = this.a.V() - 1; k2 >= 0; --k2) {
		...
		if (d0 < 0.0D) {
			for (i2 = i - b0; i2 <= i + b0; ++i2) {
				...
				for (j2 = k - b0; j2 <= k + b0; ++j2) {
					...
					label219: for (k2 = this.a.V() - 1; k2 >= 0; --k2) {
						if (this.a.isEmpty(blockposition_mutableblockposition.c(i2, k2, j2))) {
							while (k2 > 0 && this.a.isEmpty(blockposition_mutableblockposition.c(i2, k2 - 1, j2))) {
								--k2;
							}
