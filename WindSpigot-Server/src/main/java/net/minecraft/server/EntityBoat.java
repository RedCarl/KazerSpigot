...
public class EntityBoat extends Entity {
	...
	public void collide(Entity entity) {
		...
		VehicleEntityCollisionEvent event = new VehicleEntityCollisionEvent((Vehicle) this.getBukkitEntity(),
				hitEntity);
	...
	public boolean damageEntity(DamageSource damagesource, float f) {
		if (this.isInvulnerable(damagesource)) {
			...
			if (this.passenger != null && this.passenger == damagesource.getEntity()
		...
		if (d3 > 0.2975D) {
			...
			for (j = 0; j < 1.0D + d3 * 60.0D; ++j) {
				...
				if (this.random.nextBoolean()) {
					...
					this.world.addParticle(EnumParticle.WATER_SPLASH, d8, this.locY - 0.125D, d9, this.motX, this.motY,
							this.motZ, Constants.EMPTY_ARRAY);
				} else {
	...
	protected void a(double d0, boolean flag, Block block, BlockPosition blockposition) {
		...
		} else if (this.world.getType((new BlockPosition(this)).down()).getBlock().getMaterial() != Material.WATER
				&& d0 < 0.0D) {
