...
public class EntityRabbit extends EntityAnimal {
		...
		if (this.onGround) {
			...
			if (this.getRabbitType() == 99 && this.bs == 0) {
				...
				if (entityliving != null && this.h(entityliving) < 16.0D) {
					...
					this.moveController.a(entityliving.locX, entityliving.locY, entityliving.locZ,
							this.moveController.b());
	...
	public EntityRabbit b(EntityAgeable entityageable) {
		...
		if (entityageable instanceof EntityRabbit) {
			entityrabbit.setRabbitType(
					this.random.nextBoolean() ? this.getRabbitType() : ((EntityRabbit) entityageable).getRabbitType());
		}
	...
	protected void cp() {
		this.world.addParticle(EnumParticle.BLOCK_DUST,
				...
				Block.getCombinedId(Blocks.CARROTS.fromLegacyData(7)));
		this.bu = 100;
	}
	...
	static class PathfinderGoalEatCarrots extends PathfinderGoalGotoTarget {
		...
		public void e() {
			...
			if (this.f()) {
				...
				if (this.e && block instanceof BlockCarrots && iblockdata.get(BlockCrops.AGE) == 7) {
		...
		protected boolean a(World world, BlockPosition blockposition) {
			...
			if (block == Blocks.FARMLAND) {
				...
				if (block instanceof BlockCarrots && iblockdata.get(BlockCrops.AGE) == 7 && this.d
						&& !this.e) {
					this.e = true;
					return true;
				}
