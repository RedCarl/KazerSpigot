...
public class EntityArmorStand extends EntityLiving {
	...
	public boolean d(int i, ItemStack itemstack) {
		...
		if (itemstack != null && EntityInsentient.c(itemstack) != j
	...
	public boolean a(EntityHuman entityhuman, Vec3D vec3d) {
		if (this.s()) {
			...
			if (d4 >= 0.1D && d4 < 0.1D + (flag1 ? 0.8D : 0.45D) && this.items[1] != null) {
				b1 = 1;
			} else if (d4 >= 0.9D + (flag1 ? 0.3D : 0.0D) && d4 < 0.9D + (flag1 ? 1.0D : 0.7D)
					&& this.items[3] != null) {
				b1 = 3;
	...
	private void a(EntityHuman entityhuman, int i) {
		...
		if (itemstack == null || (this.bi & 1 << i + 8) == 0) {
			if (itemstack != null || (this.bi & 1 << i + 16) == 0) {
				...
				PlayerArmorStandManipulateEvent armorStandManipulateEvent = new PlayerArmorStandManipulateEvent(player,
						self, playerHeldItem, armorStandItem, slot);
				...
				if (entityhuman.abilities.canInstantlyBuild
						&& (itemstack == null || itemstack.getItem() == Item.getItemOf(Blocks.AIR))
						&& itemstack1 != null) {
			...
			if (damagesource.isExplosion()) {
				...
				if (!flag1 && !flag) {
					...
					if (damagesource.getEntity() instanceof EntityHuman
							&& !((EntityHuman) damagesource.getEntity()).abilities.mayBuild) {
						return false;
