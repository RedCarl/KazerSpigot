...
public class EntityWither extends EntityMonster implements IRangedEntity {
	...
	public void m() {
		...
		for (j = 0; j < 3; ++j) {
			...
			this.world.addParticle(EnumParticle.SMOKE_NORMAL, d8 + this.random.nextGaussian() * 0.30000001192092896D,
					...
					d10 + this.random.nextGaussian() * 0.30000001192092896D, 0.0D, 0.0D, 0.0D, Constants.EMPTY_ARRAY);
			if (flag && this.world.random.nextInt(4) == 0) {
				this.world.addParticle(EnumParticle.SPELL_MOB, d8 + this.random.nextGaussian() * 0.30000001192092896D,
						d9 + this.random.nextGaussian() * 0.30000001192092896D,
						d10 + this.random.nextGaussian() * 0.30000001192092896D, 0.699999988079071D, 0.699999988079071D,
						0.5D, Constants.EMPTY_ARRAY);
			}
		}
	...
	protected void E() {
		...
		if (this.cl() > 0) {
			...
			if (i <= 0) {
				// CraftBukkit start
				// this.world.createExplosion(this, this.locX, this.locY + (double)
				// this.getHeadHeight(), this.locZ, 7.0F, false,
				// this.world.getGameRules().getBoolean("mobGriefing"));
				ExplosionPrimeEvent event = new ExplosionPrimeEvent(this.getBukkitEntity(), 7.0F, false);
				this.world.getServer().getPluginManager().callEvent(event);

				if (!event.isCancelled()) {
					this.world.createExplosion(this, this.locX, this.locY + this.getHeadHeight(), this.locZ,
							event.getRadius(), event.getFire(), this.world.getGameRules().getBoolean("mobGriefing"));
				}
				// CraftBukkit end
			...
			for (i = 1; i < 3; ++i) {
				if (this.ticksLived >= this.bn[i - 1]) {
					...
					if (j > 0) {
						...
						List<EntityLiving> list = this.world.a(EntityLiving.class, this.getBoundingBox().grow(20.0D, 8.0D, 20.0D),
