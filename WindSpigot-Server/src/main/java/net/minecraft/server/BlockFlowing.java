...
public class BlockFlowing extends BlockFluids {
	...
	private void f(World world, BlockPosition blockposition, IBlockData iblockdata) {
		world.setTypeAndData(blockposition,
				b(this.material).getBlockData().set(BlockFluids.LEVEL, iblockdata.get(BlockFluids.LEVEL)), 2);
	}
		...
		if (this.h(world, blockposition.down(), iblockdata2)) {
			...
			if (!event.isCancelled()) {
				if ((this.material == Material.LAVA)
						...
						&& (world.nachoSpigotConfig.enableLavaToCobblestone)) {
			...
			for (EnumDirection enumdirection1 : set) {
				...
				if (!event.isCancelled()) {
					this.flow(world, blockposition.shift(enumdirection1),
							world.getType(blockposition.shift(enumdirection1)), k);
				}
				// CraftBukkit end
			}
		}
	...
	private void flow(World world, BlockPosition blockposition, IBlockData iblockdata, int i) {
		if (world.isLoaded(blockposition) && this.h(world, blockposition, iblockdata)) { // CraftBukkit - add isLoaded
																							// check
	...
	/**
	 * PaperSpigot - Get flow speed. Throttle if its water and flowing adjacent to
	 * lava
	 */
	public int getFlowSpeed(World world, BlockPosition blockposition) {
		if (this.getMaterial() == Material.LAVA) {
			return world.worldProvider.o() ? world.paperSpigotConfig.lavaFlowSpeedNether
					: world.paperSpigotConfig.lavaFlowSpeedNormal;
		}
	...
	/**
	 * PaperSpigot - Checks surrounding blocks to determine if block can be fast
	 * drained
	 */
		...
		if (this.material == Material.WATER) {
			if (world.paperSpigotConfig.fastDrainWater) {
				...
				if (getData(world, position.down()) < 0) {
					result = false;
				} else if (world.getType(position.north()).getBlock().getMaterial() == Material.WATER
						&& getData(world, position.north()) < data) {
					result = false;
				} else if (world.getType(position.south()).getBlock().getMaterial() == Material.WATER
						&& getData(world, position.south()) < data) {
					result = false;
				} else if (world.getType(position.west()).getBlock().getMaterial() == Material.WATER
						&& getData(world, position.west()) < data) {
					result = false;
				} else if (world.getType(position.east()).getBlock().getMaterial() == Material.WATER
						&& getData(world, position.east()) < data) {
					result = false;
				}
			}
			...
			if (world.paperSpigotConfig.fastDrainLava) {
				result = true;
				if (getData(world, position.down()) < 0
						|| world.getType(position.up()).getBlock().getMaterial() != Material.AIR) {
					result = false;
				} else if (world.getType(position.north()).getBlock().getMaterial() == Material.LAVA
						&& getData(world, position.north()) < data) {
					result = false;
				} else if (world.getType(position.south()).getBlock().getMaterial() == Material.LAVA
						&& getData(world, position.south()) < data) {
					result = false;
				} else if (world.getType(position.west()).getBlock().getMaterial() == Material.LAVA
						&& getData(world, position.west()) < data) {
					result = false;
				} else if (world.getType(position.east()).getBlock().getMaterial() == Material.LAVA
						&& getData(world, position.east()) < data) {
					result = false;
				}
			}
