...
public final class ItemStack {
	...
	public boolean placeItem(EntityHuman entityhuman, World world, BlockPosition blockposition,
		...
		if (flag && world.captureTreeGeneration && world.capturedBlockStates.size() > 0) {
			...
			Location location = new Location(world.getWorld(), blockposition.getX(), blockposition.getY(),
					blockposition.getZ());
		...
		if (flag) {
			...
			world.capturedBlockStates.clear();
			if (blocks.size() > 1) {
				placeEvent = org.bukkit.craftbukkit.event.CraftEventFactory.callBlockMultiPlaceEvent(world, entityhuman,
						blocks, blockposition.getX(), blockposition.getY(), blockposition.getZ());
			} else if (blocks.size() == 1) {
				placeEvent = org.bukkit.craftbukkit.event.CraftEventFactory.callBlockPlaceEvent(world, entityhuman,
						blocks.get(0), blockposition.getX(), blockposition.getY(), blockposition.getZ());
			}
			if (placeEvent != null && (placeEvent.isCancelled() || !placeEvent.canBuild())) {
				...
				for (BlockState blockstate : blocks) {
					...
					world.notifyAndUpdatePhysics(newblockposition, null, oldBlock, block.getBlock(), updateFlag); // send
																													// null
																													// chunk
																													// as
																													// chunk.k()
				...
				// Special case juke boxes as they update their tile entity. Copied from
				// ItemRecord.
	...
	public NBTTagCompound save(NBTTagCompound nbttagcompound) {
		...
		if (this.tag != null) {
			nbttagcompound.set("tag", this.tag.clone()); // CraftBukkit - make defensive copy, data is going to another
															// thread
		}
	...
	public void c(NBTTagCompound nbttagcompound) {
		...
		/*
		 * CraftBukkit start - Route through setData for filtering this.damage =
		 * nbttagcompound.getShort("Damage"); if (this.damage < 0) { this.damage = 0; }
		 */
		this.setData(nbttagcompound.getShort("Damage"));
		// CraftBukkit end

		if (nbttagcompound.hasKeyOfType("tag", 10)) {
			// CraftBukkit - make defensive copy as this data may be coming from the save
			// thread
	...
	public void setData(int i) {
		...
		// The crafting system uses this value for a special purpose so we have to allow
		// it
		if (i == 32767) {
			this.damage = i;
			return;
		...
		if (CraftMagicNumbers.getBlock(id) != Blocks.AIR) {
			...
			if ((PaperSpigotConfig.dataValueAllowedItems == null
					|| !PaperSpigotConfig.dataValueAllowedItems.contains(id))
		...
		if (!this.e()) {
			...
			if (i > 0) {
				...
				if (entityliving instanceof EntityPlayer) {
					org.bukkit.craftbukkit.inventory.CraftItemStack item = org.bukkit.craftbukkit.inventory.CraftItemStack
							.asCraftMirror(this);
				...
				if (this.isDamaged(i, entityliving.bc(), entityliving)) { // Spigot
					...
					if (this.count == 0 && entityliving instanceof EntityHuman) {
						org.bukkit.craftbukkit.event.CraftEventFactory
								.callPlayerItemBreakEvent((EntityHuman) entityliving, this);
					}
					// CraftBukkit end
	...
	public static boolean fastMatches(ItemStack itemstack, ItemStack itemstack1) {
		...
		if (itemstack != null && itemstack1 != null) {
			return itemstack.count == itemstack1.count && itemstack.item == itemstack1.item
					&& itemstack.damage == itemstack1.damage;
		}
		return false;
	}
	// Spigot End
	...
	public Multimap<String, AttributeModifier> B() {
		...
		if (this.hasTag() && this.tag.hasKeyOfType("AttributeModifiers", 9)) {
			...
			for (int i = 0; i < nbttaglist.size(); ++i) {
				...
				if (attributemodifier != null && attributemodifier.a().getLeastSignificantBits() != 0L
	...
	public IChatBaseComponent C() {
		...
		if (this.item != null) {
			...
			ichatbasecomponent.getChatModifier().setChatHoverable(new ChatHoverable(
					ChatHoverable.EnumHoverAction.SHOW_ITEM, new ChatComponentText(nbttagcompound.toString())));
