...
public class EntityEnderDragon extends EntityInsentient implements IComplex, IMonster {
	...
	public double[] b(int i, float f) {
		...
		d1 = this.bk[k][1] - d0;
		adouble[1] = d0 + d1 * f;
		adouble[2] = this.bk[j][2] + (this.bk[k][2] - this.bk[j][2]) * f;
		return adouble;
	}
			...
			if (this.ce()) {
				...
				if (this.world.isClientSide) {
					if (this.bc > 0) {
						...
						d2 = MathHelper.g(this.bg - this.yaw);
				...
				this.bo.t_();
				this.bo.setPositionRotation(this.locX + f11 * 0.5F, this.locY, this.locZ - f12 * 0.5F, 0.0F, 0.0F);
				this.bs.t_();
				this.bs.setPositionRotation(this.locX + f12 * 4.5F, this.locY + 2.0D, this.locZ + f11 * 4.5F, 0.0F,
						0.0F);
				this.bt.t_();
				...
				if (!this.world.isClientSide && this.hurtTicks == 0) {
					...
					this.a(this.world.getEntities(this,
							this.bt.getBoundingBox().grow(4.0D, 2.0D, 4.0D).c(0.0D, -2.0D, 0.0D)));
	...
	protected void aZ() {
		...
		if (this.by >= 180 && this.by <= 200) {
			...
			this.world.addParticle(EnumParticle.EXPLOSION_HUGE, this.locX + f, this.locY + 2.0D + f1, this.locZ + f2,
					0.0D, 0.0D, 0.0D, Constants.EMPTY_ARRAY);
		}
	...
	private void a(BlockPosition blockposition) {
		...
		// CraftBukkit start - Replace any "this.world" in the following with just
		// "world"!
		...
		world.setTypeUpdate(blockposition2.west(),
				Blocks.TORCH.getBlockData().set(BlockTorch.FACING, EnumDirection.EAST));
		world.setTypeUpdate(blockposition2.east(),
				Blocks.TORCH.getBlockData().set(BlockTorch.FACING, EnumDirection.WEST));
		world.setTypeUpdate(blockposition2.north(),
				Blocks.TORCH.getBlockData().set(BlockTorch.FACING, EnumDirection.SOUTH));
		world.setTypeUpdate(blockposition2.south(),
				Blocks.TORCH.getBlockData().set(BlockTorch.FACING, EnumDirection.NORTH));
