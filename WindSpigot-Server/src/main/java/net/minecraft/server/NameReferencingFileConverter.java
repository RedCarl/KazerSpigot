...
public class NameReferencingFileConverter {
		...
		if (NameReferencingFileConverter.b.exists() && NameReferencingFileConverter.b.isFile()) {
			if (gameprofilebanlist.c().exists()) {
				try {
					...
					// CraftBukkit start - FileNotFoundException -> IOException, don't print
					// stacetrace
				} catch (IOException filenotfoundexception) {
					NameReferencingFileConverter.e
							.warn("Could not load existing file " + gameprofilebanlist.c().getName());
				}
			}
			...
			try {
				...
				ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
					...
					public void onProfileLookupSucceeded(GameProfile gameprofile) {
						...
						if (astring == null) {
							NameReferencingFileConverter.e
									.warn("Could not convert user banlist entry for " + gameprofile.getName());
					...
					public void onProfileLookupFailed(GameProfile gameprofile, Exception exception) {
						...
						if (!(exception instanceof ProfileNotFoundException)) {
							throw new NameReferencingFileConverter.FileConversionException(
									"Could not request user " + gameprofile.getName() + " from backend systems",
									exception, null);
						}
					}
				};
				...
				NameReferencingFileConverter.e.error("Conversion failed, please try again later",
						namereferencingfileconverter_fileconversionexception);
				return false;
			}
		} else {
			return true;
			...
			try {
				...
				ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
					...
					public void onProfileLookupSucceeded(GameProfile gameprofile) {
						...
						if (uuid == null) {
							throw new NameReferencingFileConverter.FileConversionException(
									"Missing UUID for user profile " + gameprofile.getName(), null);
						} else {
					...
					private void a(File file, String s, String s1) {
						...
						if (!file1.renameTo(file3)) {
							throw new NameReferencingFileConverter.FileConversionException(
									"Could not convert file for " + s, null);
						}
					}
					...
					private String a(GameProfile gameprofile) {
						...
						if (s == null) {
							throw new NameReferencingFileConverter.FileConversionException(
									"Could not find the filename for " + gameprofile.getName() + " anymore", null);
						} else {
							return s;
						}
					}
	...
	private static File d(PropertyManager propertymanager) {
		...
		File file = new File(MinecraftServer.getServer().server.getWorldContainer(), s); // CraftBukkit - Respect
																							// container setting
