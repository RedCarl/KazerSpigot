...
public class TileEntityBrewingStand extends TileEntityContainer implements IUpdatePlayerListBox, IWorldInventory {
		...
		if (this.n()) {
			...
			if (getOwner() != null) {
				BrewEvent event = new BrewEvent(
						...
						(org.bukkit.inventory.BrewerInventory) this.getOwner().getInventory());
	...
	private int c(int i, ItemStack itemstack) {
		return itemstack == null ? i
				: (itemstack.getItem().l(itemstack) ? PotionBrewer.a(i, itemstack.getItem().j(itemstack)) : i);
	}
	...
	public boolean b(int i, ItemStack itemstack) {
		return i == 3 ? itemstack.getItem().l(itemstack)
				: itemstack.getItem() == Items.POTION || itemstack.getItem() == Items.GLASS_BOTTLE;
	}
