...
public class EntityFallingBlock extends Entity {
	...
	public EntityFallingBlock(org.bukkit.Location loc, World world, double d0, double d1, double d2,
		...
		if (block.getMaterial() == Material.AIR) {
			...
			if (this.ticksLived++ == 0) {
				...
				if (this.world.getType(blockposition).getBlock() == block
						&& !CraftEventFactory.callEntityChangeBlockEvent(this, blockposition.getX(),
								blockposition.getY(), blockposition.getZ(), Blocks.AIR, 0).isCancelled()) {
			...
			if (this.world.paperSpigotConfig.fallingBlockHeightNerf != 0
					&& this.locY > this.world.paperSpigotConfig.fallingBlockHeightNerf) {
				if (this.dropItem) {
			...
			if (!this.world.isClientSide) {
					...
					if (this.world.getType(blockposition).getBlock() != Blocks.PISTON_EXTENSION) {
						...
						if (!this.e) {
							if (this.world
									...
									&& blockposition
											.getY() < (this.world.tacoSpigotConfig.disableFallingBlockStackingAt256
													? 255
													: 256)
									&& this.world.getType(blockposition) != this.block) {
								if (CraftEventFactory.callEntityChangeBlockEvent(this, blockposition.getX(),
										blockposition.getY(), blockposition.getZ(), this.block.getBlock(),
										this.block.getBlock().toLegacyData(this.block)).isCancelled()) {
									return;
								}
				...
				} else if (this.ticksLived > 100 && !this.world.isClientSide
						&& (blockposition.getY() < 1 || blockposition.getY() > 256) || this.ticksLived > 600) {
