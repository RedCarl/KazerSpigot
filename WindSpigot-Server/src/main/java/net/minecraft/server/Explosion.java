...
public class Explosion {
	...
	public void a() {
		...
		if (!this.world.tacoSpigotConfig.optimizeLiquidExplosions || !b.getMaterial().isLiquid()) { // TacoSpigot - skip
																									// calculating what
																									// blocks to blow up
																									// in water/lava
	...
	private void processEntityKnockback(Entity entity, double d7, double finalD, double finalD1, double finalD11, float f3, double d12) {
		...
		// entity.damageEntity(DamageSource.explosion(this), (float) ((int) ((d13 * d13
		// + d13) / 2.0D * 8.0D * (double) f3 + 1.0D))); // CraftBukkit start
		...
		double d14 = entity instanceof EntityHuman
				&& world.paperSpigotConfig.disableExplosionKnockback ? 0
						: EnchantmentProtection.a(entity, d13); // PaperSpigot

		// PaperSpigot start - Fix cannons
		// This impulse method sets the dirty flag, so clients will get an immediate
		// velocity update
		...
		if (entity instanceof EntityHuman
				...
				&& !world.paperSpigotConfig.disableExplosionKnockback) { // PaperSpigot
	...
	public void a(boolean flag) {
		...
		if (this.b) {
			...
			while (iterator.hasNext()) {
				...
				/*
				 * if (flag) { double d0 = (float) blockposition.getX() +
				 * this.world.random.nextFloat(); double d1 = (float) blockposition.getY() +
				 * this.world.random.nextFloat(); double d2 = (float) blockposition.getZ() +
				 * this.world.random.nextFloat(); double d3 = d0 - this.posX; double d4 = d1 -
				 * this.posY; double d5 = d2 - this.posZ; double d6 = MathHelper.sqrt(d3 * d3 +
				 * d4 * d4 + d5 * d5);
				 * 
				 * d3 /= d6; d4 /= d6; d5 /= d6; double d7 = 0.5D / (d6 / (double) this.size +
				 * 0.1D);
				 * 
				 * d7 *= this.world.random.nextFloat() * this.world.random.nextFloat() + 0.3F;
				 * d3 *= d7; d4 *= d7; d5 *= d7;
				 * this.world.addParticle(EnumParticle.EXPLOSION_NORMAL, (d0 + this.posX) /
				 * 2.0D, (d1 + this.posY) / 2.0D, (d2 + this.posZ) / 2.0D, d3, d4, d5);
				...
				if (position.getX() != floorX || position.getY() != floorY || position.getZ() != floorZ) {
					...
					if (block != Blocks.AIR) {
						...
						if (f > 0.0F
								...
								&& position.getY() < 256 && position.getY() >= 0) { // CraftBukkit - don't wrap
																					// explosions
	...
	/*
	 * :: IonSpigot - comment this out static class CacheKey { private final World
	 * world; private final double posX, posY, posZ; private final double minX,
	 * minY, minZ; private final double maxX, maxY, maxZ;
	 * 
	 * public CacheKey(Explosion explosion, AxisAlignedBB aabb) { this.world =
	 * explosion.world; this.posX = explosion.posX; this.posY = explosion.posY;
	 * this.posZ = explosion.posZ; this.minX = aabb.getMinX(); this.minY =
	 * aabb.getMinY(); this.minZ = aabb.getMinZ(); this.maxX = aabb.getMaxX();
	 * this.maxY = aabb.getMaxY(); this.maxZ = aabb.getMaxZ(); }
	 * 
	 ...
	 * 
	 * if (Double.compare(cacheKey.posX, posX) != 0) return false; if
	 * (Double.compare(cacheKey.posY, posY) != 0) return false; if
	 * (Double.compare(cacheKey.posZ, posZ) != 0) return false; if
	 * (Double.compare(cacheKey.minX, minX) != 0) return false; if
	 * (Double.compare(cacheKey.minY, minY) != 0) return false; if
	 * (Double.compare(cacheKey.minZ, minZ) != 0) return false; if
	 * (Double.compare(cacheKey.maxX, maxX) != 0) return false; if
	 * (Double.compare(cacheKey.maxY, maxY) != 0) return false; if
	 * (Double.compare(cacheKey.maxZ, maxZ) != 0) return false; return
	 * world.equals(cacheKey.world); }
	 * 
	 * @Override public int hashCode() { int result; long temp; result =
	 * world.hashCode(); temp = Double.doubleToLongBits(posX); result = 31 * result
	 * + (int) (temp ^ (temp >>> 32)); temp = Double.doubleToLongBits(posY); result
	 * = 31 * result + (int) (temp ^ (temp >>> 32)); temp =
	 * Double.doubleToLongBits(posZ); result = 31 * result + (int) (temp ^ (temp >>>
	 * 32)); temp = Double.doubleToLongBits(minX); result = 31 * result + (int)
	 * (temp ^ (temp >>> 32)); temp = Double.doubleToLongBits(minY); result = 31 *
	 * result + (int) (temp ^ (temp >>> 32)); temp = Double.doubleToLongBits(minZ);
	 * result = 31 * result + (int) (temp ^ (temp >>> 32)); temp =
	 * Double.doubleToLongBits(maxX); result = 31 * result + (int) (temp ^ (temp >>>
	 * 32)); temp = Double.doubleToLongBits(maxY); result = 31 * result + (int)
	 * (temp ^ (temp >>> 32)); temp = Double.doubleToLongBits(maxZ); result = 31 *
