...
public class ItemBucket extends Item {
	...
	public ItemStack a(ItemStack itemstack, World world, EntityHuman entityhuman) {
		...
		if (movingobjectposition == null) {
			...
			if (movingobjectposition.type == MovingObjectPosition.EnumMovingObjectType.BLOCK) {
				...
				if (flag) {
					if (!entityhuman.a(blockposition.shift(movingobjectposition.direction),
					...
					if (material == Material.WATER && iblockdata.get(BlockFluids.LEVEL) == 0) {
						...
						return this.a(itemstack, entityhuman, Items.WATER_BUCKET, event.getItemStack()); // CraftBukkit
																											// - added
																											// Event
																											// stack
					}
					...
					if (material == Material.LAVA && iblockdata.get(BlockFluids.LEVEL) == 0) {
						...
						world.setAir(blockposition);
						entityhuman.b(StatisticList.USE_ITEM_COUNT[Item.getId(this)]);
						return this.a(itemstack, entityhuman, Items.LAVA_BUCKET, event.getItemStack()); // CraftBukkit -
																										// added Event
																										// stack
					}
				} else {
	...
	// CraftBukkit - added ob.ItemStack result - TODO: Is this... the right way to
	// handle this?
				...
				if (world.worldProvider.n() && this.a == Blocks.FLOWING_WATER) {
					...
					for (int l = 0; l < 8; ++l) {
						world.addParticle(EnumParticle.SMOKE_LARGE, i + Math.random(), j + Math.random(),
								k + Math.random(), 0.0D, 0.0D, 0.0D, Constants.EMPTY_ARRAY);
					}
				} else {
