...
public class WorldServer extends World implements IAsyncTaskHandler {
    ...
    private static final List<StructurePieceTreasure> U = Lists.newArrayList(
            ...
            new StructurePieceTreasure(Items.APPLE, 0, 2, 3, 5), new StructurePieceTreasure(Items.BREAD, 0, 2, 3, 3),
    ...
    // private final Set<NextTickListEntry> L = Sets.newHashSet(); // PAIL: Rename
    // nextTickListHash
    ...
    private final WorldServer.BlockActionDataList[] S = new WorldServer.BlockActionDataList[]{
            new WorldServer.BlockActionDataList(null), new WorldServer.BlockActionDataList(null)};
    private int T;
    ...
    public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, WorldData worlddata, int i,
        ...
        this.Q = ((org.bukkit.craftbukkit.CraftTravelAgent) new org.bukkit.craftbukkit.CraftTravelAgent(this)
                .setSearchRadius(paperSpigotConfig.portalSearchRadius)); // CraftBukkit // Paper - configurable search
        // radius
        this.B();
        this.C();
    ...
    public World b() {
        ...
        if (getServer().getScoreboardManager() == null) { // CraftBukkit
            ...
            PersistentScoreboard persistentscoreboard = (PersistentScoreboard) this.worldMaps
                    .get(PersistentScoreboard.class, "scoreboard");
    ...
    public void doTick() {
        ...
        // CraftBukkit start - Only call spawner if we have players online and the world
        // allows for mobs or animals
    ...
    public boolean everyoneDeeplySleeping() {
        ...
        /*
         * if (this.O && !this.isClientSide) { Iterator<EntityHuman> iterator =
         * this.players.iterator();
         *
         * // CraftBukkit - This allows us to assume that some people are in bed but not
         * really, allowing time to pass in spite of AFKers boolean foundActualSleepers
         * = false;
         *
         * EntityHuman entityhuman;
         *
         * do { if (!iterator.hasNext()) { return foundActualSleepers; }
         *
         * entityhuman = iterator.next();
         *
         * // CraftBukkit start if (entityhuman.isDeeplySleeping()) {
         * foundActualSleepers = true; } } while (!entityhuman.isSpectator() ||
         * entityhuman.isDeeplySleeping() || entityhuman.fauxSleeping); // CraftBukkit
         * end
         *
         * } return false;
         */
    ...
    protected void h() {
        ...
        if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
            ...
            // CraftBukkit start
            // for (Iterator iterator1 = this.chunkTickList.iterator(); iterator1.hasNext();
            // this.methodProfiler.b()) {
            ...
            for (gnu.trove.iterator.TLongShortIterator iter = chunkTickList.iterator(); iter.hasNext(); ) {
                ...
                if ((!this.chunkProvider.isChunkLoaded(chunkX, chunkZ))
                        || (this.chunkProviderServer.unloadQueue.contains(LongHash.toLong(chunkX, chunkZ)))) // TacoSpigot
                // -
                // invoke
                // LongHash
                // directly
                ...
                if (!this.paperSpigotConfig.disableThunder && this.random.nextInt(100000) == 0 && this.S()
                        && this.R()) { // PaperSpigot - Disable thunder
                ...
                if (!this.paperSpigotConfig.disableIceAndSnow && this.random.nextInt(16) == 0) { // PaperSpigot -
                    // Disable ice and
                    // snow
                    this.m = this.m * 3 + 1013904223;
                    i1 = this.m >> 2;
                        ...
                        for (ChunkSection section : chunk.getSections()) {
                            if (section != null && section.shouldTick()) {
                                for (int l1 = 0; l1 < randomTickSpeed; ++l1) {
                                    ...
                                    if (block.isTicking()) {
                                        ++i;
                                        block.a(this, new BlockPosition(j2 + k, l2 + section.getYPosition(), k2 + l),
    ...
    protected BlockPosition a(BlockPosition blockposition) {
        ...
        AxisAlignedBB axisalignedbb = (new AxisAlignedBB(blockposition1,
                new BlockPosition(blockposition1.getX(), this.getHeight(), blockposition1.getZ()))).grow(3.0D, 3.0D,
                3.0D);
        List list = this.a(EntityLiving.class, axisalignedbb, new Predicate() {
            public boolean a(EntityLiving entityliving) {
                return entityliving != null && entityliving.isAlive()
                        && WorldServer.this.i(entityliving.getChunkCoordinates());
            }
        ...
        return !list.isEmpty() ? ((EntityLiving) list.get(this.random.nextInt(list.size()))).getChunkCoordinates()
                : blockposition1;
    }
            ...
            if (block.N()) {
                ...
                if (this.areChunksLoadedBetween(nextticklistentry.a.a(-b0, -b0, -b0),
                    ...
                    if (iblockdata.getBlock().getMaterial() != Material.AIR
                            && iblockdata.getBlock() == nextticklistentry.a()) {
     ...
     * needed public void entityJoinedWorld(Entity entity, boolean flag) { if
     * (!this.getSpawnAnimals() && (entity instanceof EntityAnimal || entity
     * instanceof EntityWaterAnimal)) { entity.die(); }
     *
     * if (!this.getSpawnNPCs() && entity instanceof NPC) { entity.die(); }
     *
     * super.entityJoinedWorld(entity, flag); } // CraftBukkit end
     */
    ...
    public List<TileEntity> getTileEntities(int i, int j, int k, int l, int i1, int j1) {
        ...
        /*
         * for (int k1 = 0; k1 < this.h.size(); ++k1) { TileEntity tileentity =
         * (TileEntity) this.h.get(k1); BlockPosition blockposition =
         * tileentity.getPosition();
         *
         * if (blockposition.getX() >= i && blockposition.getY() >= j &&
         * blockposition.getZ() >= k && blockposition.getX() < l && blockposition.getY()
         * < i1 && blockposition.getZ() < j1) { arraylist.add(tileentity); } }
         */
        // CraftBukkit end
    ...
    public boolean strikeLightning(Entity entity) {
        ...
        LightningStrikeEvent lightning = new LightningStrikeEvent(this.getWorld(),
                (org.bukkit.entity.LightningStrike) entity.getBukkitEntity());
    ...
    protected void p() {
        ...
        super.p();
        /*
         * CraftBukkit start if (this.o != this.p) { this.server.getPlayerList().a(new
         * PacketPlayOutGameStateChange(7, this.p), this.worldProvider.getDimension());
         * }
         *
         * if (this.q != this.r) { this.server.getPlayerList().a(new
         * PacketPlayOutGameStateChange(8, this.r), this.worldProvider.getDimension());
         * }
         *
         * if (flag != this.S()) { if (flag) { this.server.getPlayerList().sendAll(new
         * PacketPlayOutGameStateChange(2, 0.0F)); } else {
         * this.server.getPlayerList().sendAll(new PacketPlayOutGameStateChange(1,
         * 0.0F)); }
         *
         * this.server.getPlayerList().sendAll(new PacketPlayOutGameStateChange(7,
         * this.p)); this.server.getPlayerList().sendAll(new
         * PacketPlayOutGameStateChange(8, this.r)); } //
         */
        if (flag != this.S()) {
