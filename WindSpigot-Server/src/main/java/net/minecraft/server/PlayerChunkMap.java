...
public class PlayerChunkMap {
	...
	private final Queue<PlayerChunkMap.PlayerChunk> f = new java.util.concurrent.ConcurrentLinkedQueue<>(); // visibleChunks
																											// //
																											// CraftBukkit
																											// ArrayList
																											// ->
	...
	public void flush() {
		...
		if (i - this.h > 8000L) {
			this.h = i;
			// Nacho - or just use an enhanced for loop here :shrug: // CraftBukkit start -
			// Use iterator
	...
	public boolean a(EntityPlayer entityplayer, int i, int j) {
		...
		return chunk != null && chunk.b.contains(entityplayer)
				&& !entityplayer.chunkCoordIntPairQueue.contains(chunk.location);
	}
	...
	class PlayerChunk {
		...
		public void a(final EntityPlayer entityplayer) { // CraftBukkit - added final to argument
			if (this.b.contains(entityplayer)) {
				PlayerChunkMap.a.debug("Failed to add player. {} already is in chunk {}, {}", entityplayer,
			...
			if (this.dirtyCount != 0) {
				...
				if (this.dirtyCount == 1) {
					...
					if (this.dirtyCount == 64) {
						...
						this.a(new PacketPlayOutMapChunk(
								PlayerChunkMap.this.world.getChunkAt(this.location.x, this.location.z), false, this.f));

						for (k = 0; k < 16; ++k) {
							if ((this.f & 1 << k) != 0) {
								l = k << 4;
								List<TileEntity> list = PlayerChunkMap.this.world.getTileEntities(i, l, j, i + 16,
										l + 16, j + 16);
