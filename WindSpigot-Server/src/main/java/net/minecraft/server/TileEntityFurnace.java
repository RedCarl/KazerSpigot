			...
			if (!this.isBurning() && (this.items[1] == null || this.items[0] == null)) {
				...
				if (this.burnTime <= 0 && this.canBurn()) { // CraftBukkit - == to <=
					...
					FurnaceBurnEvent furnaceBurnEvent = new FurnaceBurnEvent(
							this.world.getWorld().getBlockAt(position.getX(), position.getY(), position.getZ()), fuel,
							fuelTime(this.items[1]));
				...
				/*
				 * CraftBukkit start - Moved up if (this.isBurning() && this.canBurn()) {
				 * ++this.cookTime; if (this.cookTime == this.cookTimeTotal) { this.cookTime =
				 * 0; this.cookTimeTotal = this.a(this.items[0]); this.burn(); flag1 = true; } }
	...
	public void burn() {
		if (this.canBurn()) {
			...
			FurnaceSmeltEvent furnaceSmeltEvent = new FurnaceSmeltEvent(
					this.world.getWorld().getBlockAt(position.getX(), position.getY(), position.getZ()), source,
					result);
			...
			/*
			 * if (this.items[2] == null) { this.items[2] = itemstack.cloneItemStack(); }
			 * else if (this.items[2].getItem() == itemstack.getItem()) {
			 * ++this.items[2].count; }
			 */
			// CraftBukkit end

			if (this.items[0].getItem() == Item.getItemOf(Blocks.SPONGE) && this.items[0].getData() == 1
					&& this.items[1] != null && this.items[1].getItem() == Items.BUCKET) {
