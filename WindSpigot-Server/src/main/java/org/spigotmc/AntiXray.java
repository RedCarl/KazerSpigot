...
public class AntiXray {
	...
	public void obfuscate(int chunkX, int chunkY, int bitmask, byte[] buffer, World world) {
		...
		if (world.spigotConfig.antiXray) {
			...
			BlockPosition.MutableBlockPosition pos = new BlockPosition.MutableBlockPosition(); // TacoSpigot -
																								// preallocate
						...
						for (int z = 0; z < 16; z++) {
							for (int x = 0; x < 16; x++) {
								...
								if (obfuscateBlocks[blockId]) {
									...
									// On the otherhand, if radius is 0, or the nearby blocks are all non air, we
									// can obfuscate
