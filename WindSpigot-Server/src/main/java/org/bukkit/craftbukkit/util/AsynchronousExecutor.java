 ...
 * {@link AsynchronousExecutor#get(Object)} methods. <li \> Stage 1 creates the
 * object from a parameter, and is usually called asynchronously. <li \> Stage 2
 * takes the parameter and object from stage 1 and does any synchronous
 * processing to prepare it. <li \> Stage 3 takes the parameter and object from
 * stage 1, as well as a callback that was registered, and performs any
...
public final class AsynchronousExecutor<P, T, C, E extends Throwable> {
	 ...
	 * execution. Stage 3 will have no callbacks, stage 2 will be skipped unless a
