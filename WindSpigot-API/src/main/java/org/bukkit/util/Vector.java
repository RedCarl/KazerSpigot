...
public class Vector implements Cloneable, ConfigurationSerializable {
	...
	/**
	 * Gets the magnitude of the vector, defined as sqrt(x^2+y^2+z^2). The value of
	 * this method is not cached and uses a costly square-root function, so do not
	...
	/**
	 * Get the distance between this vector and another. The value of this method is
	 * not cached and uses a costly square-root function, so do not repeatedly call
	 * this method to get the vector's magnitude. NaN will be returned if the inner
	 * result of the sqrt() function overflows, which will be caused if the distance
	 * is too long.
	 *
	 ...
	 * defined as x1*x2+y1*y2+z1*z2. The returned value is a scalar.
	 *
	 * @param other The other vector
	 * @return dot product
	 */
	...
	/**
	 * Calculates the cross product of this vector with another. The cross product
	 * is defined as:
	 * <ul>
	 * <li>x = y1 * z2 - y2 * z1
	 * <li>y = z1 * x2 - z2 * x1
	 ...
	 * <p>
	 * The minimum and maximum vectors given must be truly the minimum and maximum
	 ...
	 * <p>
	 * This piece of math is based on the standard rotation matrix for vectors in
	 * three dimensional space. This matrix can be found here: <a href=
	 * "https://en.wikipedia.org/wiki/Rotation_matrix#Basic_rotations">Rotation
	 * Matrix</a>.
	 *
	 * @param angle the angle to rotate the vector about. This angle is passed in
	 *              radians
	 * @return the same vector
	 */
	 ...
	 *
	 * <p>
	 * Rotation will follow the general Right-Hand-Rule, which means rotation will
	 * be counterclockwise when the axis is pointing towards the observer.
	 * <p>
	 * This method will always make sure the provided axis is a unit vector, to not
	 * modify the length of the vector when rotating. If you are experienced with
	 * the scaling of a non-unit axis vector, you can use
	 * {@link Vector#rotateAroundNonUnitAxis(Vector, double)}.
	 *
	 * @param axis  the axis to rotate the vector around. If the passed vector is
	 *              not of length 1, it gets copied and normalized before using it
	 *              for the rotation. Please use {@link Vector#normalize()} on the
	 ...
	 * length. If the provided axis is not a unit vector, the rotated vector will
	 * not have its previous length. The scaled length of the resulting vector will
	 * be related to the axis vector. If you are not perfectly sure about the
	 * scaling of the vector, use {@link Vector#rotateAroundAxis(Vector, double)}
	 *
	 ...
	 * account for floating point errors. The epsilon can be retrieved with epsilon.
	 */
	@Override
