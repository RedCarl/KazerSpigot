...
public class BlockIterator implements Iterator<Block> {
	...
	public BlockIterator(World world, Vector start, Vector direction, double yOffset, int maxDistance) {
		...
		// trace line backwards to find intercept with plane perpendicular to the main
		// axis
		...
		// This means that when the variables are positive, it means that the coord=1
		...
		maxDistanceInt = round(maxDistance / (Math.sqrt(
	 ...
	 *
	 * @param loc     The location for the start of the ray trace
	 * @param yOffset The trace begins vertically offset from the start vector by
	 *                this value
	 */
