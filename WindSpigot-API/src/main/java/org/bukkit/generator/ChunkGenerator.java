...
public abstract class ChunkGenerator {
	...
	/*
	  ...
	  generateExtBlockSections() and generateBlockSections() are unimplemented and
	  return null.
	...
	/**
	 * Shapes the chunk for the given coordinates, with extended block IDs supported
	 * (0-4095).
	 * <p>
	 * As of 1.2, chunks are represented by a vertical array of chunk sections, each
	 * of which is 16 x 16 x 16 blocks. If a section is empty (all zero), the
	 * section does not need to be supplied, reducing memory usage.
	 * <p>
	 ...
	 * </pre>
	 * 
	 * Each section {@code (sectionID = (Y>>4))} that has blocks needs to be
	 ...
	 * <p>
	 * Setting a block at X, Y, Z within the chunk can be done with the following
	 * mapping function:
	 * 
	 * <pre>
	 ...
	 * <p>
	 * This method should <b>never</b> modify a ChunkData after it has been
	 * returned.
	 * <p>
	 * This method <b>must</b> return a ChunkData returned by
	 * {@link ChunkGenerator#createChunkData(org.bukkit.World)}
	 * 
	 * @param world  The world this chunk will be used for
	 * @param random The random generator to use
	 * @param x      The X-coordinate of the chunk
	 ...
	 * <p>
	 * A null value is returned if a world should not use a fixed spawn point, and
	...
	public interface ChunkData {
		...
		/**
		 * Set a region of this chunk from xMin, yMin, zMin (inclusive) to xMax, yMax,
		 * zMax (exclusive) to block id and data.
		 * <p>
		 * Setting blocks outside the chunk's bounds does nothing.
		 *
		 * @param xMin    minimum x location (inclusive) in the chunk to set
		 ...
		 *          (exclusive)
		 * @param z the z location in the chunk from 0-15 inclusive
		 * @return the block data value or air if x, y or z are outside the chunk's
		 *         bounds
		 * @deprecated Uses magic values
		 */
		@Deprecated
