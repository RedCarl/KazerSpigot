...
/**
 * This type is the runtime-container for the information in the plugin.yml. All
 * plugins must have a respective plugin.yml. For plugins written in java using
 * the standard plugin loader, this file must be in the root of the jar file.
 * <p>
 * When Bukkit loads a plugin, it needs to know some basic information about it.
 * It reads this information from a YAML file, 'plugin.yml'. This file consists
 * of a set of attributes, each defined on a new line and with no indentation.
 * <p>
 ...
 * <td>The default {@link Permission#getDefault() default} permission state for
 * defined {@link #getPermissions() permissions} the plugin will register</td>
 * </tr>
 * <tr>
...
public final class PluginDescriptionFile {
	 ...
	 * <ul>
	 * <li>Must consist of all alphanumeric characters, underscores, hyphon, and
	 * period (a-z,A-Z,0-9, _.-). Any other character will cause the plugin.yml to
	 * fail loading.
	 * <li>Used to determine the name of the plugin's data folder. Data folders are
	 * placed in the ./plugins/ directory by default, but this behavior should not
	 * be relied on. {@link Plugin#getDataFolder()} should be used to reference the
	 * data folder.
	 ...
	 * <li>Typically you will increment this every time you release a new feature or
	 * bug fix.
	 ...
	 * successfully be resolved at runtime. For most plugins, this is the class that
	 * extends {@link JavaPlugin}.
	 * <ul>
	 * <li>This must contain the full namespace including the class file itself.
	 * <li>If your namespace is <code>org.bukkit.plugin</code>, and your class file
	 * is called <code>MyPlugin</code> then this must be
	 * <code>org.bukkit.plugin.MyPlugin</code>
	 * <li>No plugin can use <code>org.bukkit.</code> as a base package for <b>any
	 * class</b>, including the main class.
	 * </ul>
	 * <p>
	 ...
	 * {@link #getLoadBefore()} become relative in order loaded per-phase. If a
	 * plugin loads at <code>STARTUP</code>, but a dependency loads at
	 * <code>POSTWORLD</code>, the dependency will not be loaded before the plugin
	 * is loaded.
	 * </ul>
	 * <p>
	 ...
	 * <li>Used in some server error messages to provide helpful feedback on who to
	 * contact when an error occurs.
	 ...
	 * <li><code>authors</code> must be in
	 * <a href="http://en.wikipedia.org/wiki/YAML#Lists">YAML list format</a>.
	 * </ul>
	 * <p>
	 ...
	 * </pre>
	 * 
	 * </blockquote> When both are specified, author will be the first entry in the
	 * list, so this example: <blockquote>
	 * 
	 * <pre>
	 * author: Grum
	 *authors:
	 ...
	 * <ul>
	 * <li>A link to the Curse page that includes documentation and downloads is
	 * highly recommended.
	 * <li>Displayed when a user types <code>/version PluginName</code>
	 * </ul>
	 * <p>
	 ...
	 * <ul>
	 * <li>Use the value in the {@link #getName()} of the target plugin to specify
	 * the dependency.
	 * <li>If any plugin listed here is not found, your plugin will fail to load at
	 * startup.
	 * <li>If multiple plugins list each other in <code>depend</code>, creating a
	 * network with no individual plugin does not list another plugin in the <a
	 * href=https://en.wikipedia.org/wiki/Circular_dependency>network</a>, all
	 * plugins in that network will fail.
	 * <li><code>depend</code> must be in must be in
	 * <a href="http://en.wikipedia.org/wiki/YAML#Lists">YAML list format</a>.
	 * </ul>
	 * <p>
	 ...
	 * functionality. The {@link PluginManager} will make best effort to treat all
	 * entries here as if they were a {@link #getDepend() dependency}, but will
	 * never fail because of one of these entries.
	 * <ul>
	 * <li>Use the value in the {@link #getName()} of the target plugin to specify
	 * the dependency.
	 * <li>When an unresolvable plugin is listed, it will be ignored and does not
	 * affect load order.
	 * <li>When a circular dependency occurs (a network of plugins depending or
	 * soft-dependending each other), it will arbitrarily choose a plugin that can
	 * be resolved when ignoring soft-dependencies.
	 * <li><code>softdepend</code> must be in
	 * <a href="http://en.wikipedia.org/wiki/YAML#Lists">YAML list format</a>.
	 * </ul>
	 * <p>
	 ...
	 * specified plugin's {@link #getSoftDepend()} include {@link #getName() this
	 * plugin}.
	 * <li><code>loadbefore</code> must be in
	 * <a href="http://en.wikipedia.org/wiki/YAML#Lists">YAML list format</a>.
	 * </ul>
	 * <p>
	 ...
	 * corresponds to a single command and the respective values are the properties
	 * of the command. Each property, <i>with the exception of aliases</i>, can be
	 * defined at runtime using methods in {@link PluginCommand} and are defined
	 ...
	 * <td>String</td>
	 * <td>A user-friendly description for a command. It is useful for documentation
	 * purposes as well as in-game help.</td>
	 * <td><blockquote>
	 * 
	 * <pre>
	 * description: Set yourself on fire
	 ...
	 * strings</td>
	 * <td>Alternative command names, with special usefulness for commands that are
	 * already registered. <i>Aliases are not effective when defined at runtime,</i>
	 * so the plugin description file is the only way to have them properly defined.
	 * <p>
	 ...
	 * <td>String</td>
	 * <td>The name of the {@link Permission} required to use the command. A user
	 * without the permission will receive the specified message (see
	 * {@linkplain PluginCommand#setPermissionMessage(String) below}), or a standard
	 * one if no specific message is defined. Without the permission node, no
	 * {@link PluginCommand#setExecutor(CommandExecutor) CommandExecutor} or
	 * {@link PluginCommand#setTabCompleter(TabCompleter) TabCompleter} will be
	 * called.</td>
	 * <td><blockquote>
	 * 
	 * <pre>
	 ...
	 * <td>
	 * <ul>
	 * <li>Displayed to a player that attempts to use a command, but does not have
	 * the required permission. See {@link PluginCommand#getPermission() above}.
	 * <li>&lt;permission&gt; is a macro that is replaced with the permission node
	 * required to use the command.
	 * <li>Using empty quotes is a valid way to indicate nothing should be displayed
	 * to a player.
	 * </ul>
	 * </td>
	 * <td><blockquote>
	 ...
	 * <a href="http://yaml.org/spec/current.html#id2503232">surround the message
	 ...
	 * </tr>
	 * </table>
	 * The commands are structured as a hiearchy of
	 * <a href="http://yaml.org/spec/current.html#id2502325">nested mappings</a>.
	 * The primary (top-level, no intendentation) node is `<code>commands</code>',
	 * while each individual command name is indented, indicating it maps to some
	 * value (in our case, the properties of the table above).
	 * <p>
	 * Here is an example bringing together the piecemeal examples above, as well as
	 * few more definitions:<blockquote>
	 * 
	 * <pre>
	 *commands:
	 *  flagrate:
	 ...
	 * immediately proceding enabling. The format for defining permissions is a map
	 * from permission name to properties. To represent a map without any specific
	 * property, empty
	 * <a href="http://yaml.org/spec/current.html#id2502702">curly-braces</a> (
	 ...
	 * {@link Permission#getDefault()}. If not defined, it will be set to the value
	 * of {@link PluginDescriptionFile#getPermissionDefault()}.
	 * <p>
	 * For reference:
	 * <ul>
	 ...
	 * {@linkplain Permission#getChildren() relation} to the parent permission. When
	 * a parent permissions is assigned, child permissions are respectively assigned
	 * as well.
	 * <ul>
	 * <li>When a parent permission is assigned negatively, child permissions are
	 * assigned based on an inversion of their association.
	 * <li>When a parent permission is assigned positively, child permissions are
	 * assigned based on their association.
	 * </ul>
	 * <p>
	 * Child permissions may be defined in a number of ways:
	 * <ul>
	 * <li>Children may be defined as a
	 * <a href="http://en.wikipedia.org/wiki/YAML#Lists">list</a> of names. Using a
	 * list will treat all children associated positively to their parent.
	 * <li>Children may be defined as a map. Each permission name maps to either a
	 * boolean (representing the association), or a nested permission definition
	 * (just as another permission). Using a nested definition treats the child as a
	 * positive association.
	 * <li>A nested permission definition must be a map of these same properties. To
	 * define a valid nested permission without defining any specific property,
	 * empty curly-braces ( <code>&#123;&#125;</code> ) must be used.
	 * <li>A nested permission may carry it's own nested permissions as children, as
	 * they may also have nested permissions, and so forth. There is no direct limit
	 * to how deep the permission tree is defined.
	 * </ul>
	 * </td>
	 ...
	 * </pre>
	 * 
	 * </blockquote> An additional example showing basic nested values can be seen
	 * <a href="doc-files/permissions-example_plugin.yml">here</a>.</td>
	 * </tr>
	 * </table>
	 * The permissions are structured as a hiearchy of
	 * <a href="http://yaml.org/spec/current.html#id2502325">nested mappings</a>.
	 * The primary (top-level, no intendentation) node is
	 * `<code>permissions</code>', while each individual permission name is
	 * indented, indicating it maps to some value (in our case, the properties of
	 * the table above).
	 * <p>
	 ...
	 * <li>It only affects permissions that do not define the <code>default</code>
	 * node.
	 ...
	 * dictates something that a plugin developer acknowledges when the plugin is
	 * compiled. Some implementions may define extra awarenesses that are not
	 * included in the API. Any unrecognized awareness (one unsupported or in a
	 * future version) will cause a dummy object to be created instead of failing.
	 * 
	 * <ul>
	 * <li>Currently only supports the enumerated values in
	 * {@link PluginAwareness.Flags}.
	 * <li>Each awareness starts the identifier with bang-at (<code>!@</code>).
	 * <li>Unrecognized (future / unimplemented) entries are quietly replaced by a
	 * generic object that implements PluginAwareness.
	 * <li>A type of awareness must be defined by the runtime and acknowledged by
	 * the API, effectively discluding any derived type from any plugin's classpath.
	 * <li><code>awareness</code> must be in
	 * <a href="http://en.wikipedia.org/wiki/YAML#Lists">YAML list format</a>.
	 * </ul>
	 * <p>
	 ...
	 * gracefully substituted, previous versions of Bukkit (ones prior to the first
	 * implementation of awareness) will fail to load a plugin that defines any
	 * awareness.
	 *
	...
	/**
	 * Returns the name of a plugin, including the version. This method is provided
	 * for convenience; it uses the {@link #getName()} and {@link #getVersion()}
	 * entries.
	 *
