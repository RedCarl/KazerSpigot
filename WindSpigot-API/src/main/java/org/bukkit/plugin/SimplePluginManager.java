	...
	public Plugin[] loadPlugins(File directory) {
		...
		for (File file : directory.listFiles()) {
			...
			try {
				...
				if (name.equalsIgnoreCase("bukkit") || name.equalsIgnoreCase("minecraft")
					...
					server.getLogger().warning(String.format(
							...
							description.getFullName(), description.rawName));
				}
			...
			if (replacedFile != null) {
				server.getLogger().severe(String.format("Ambiguous plugin name `%s' for files `%s' and `%s' in `%s'",
						description.getName(), file.getPath(), replacedFile.getPath(), directory.getPath()));
			}
	...
	public void disablePlugin(final Plugin plugin) {
		if (plugin.isEnabled()) {
			...
			try {
				...
				server.getLogger().log(Level.SEVERE, "Error occurred (in the plugin loader) while cancelling tasks for "
						+ plugin.getDescription().getFullName() + " (Is it up to date?)", ex);
			}
