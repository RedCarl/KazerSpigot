...
public interface Player extends HumanEntity, Conversable, CommandSender, OfflinePlayer, PluginMessageRecipient {
	 ...
	 * appear as two different people. If a player joins afterwards with a name that
	 * conflicts with a player's custom list name, the joining player's player list
	 * name will have a random number appended to it (1-2 characters long in the
	 * default implementation). If the joining player's name is 15 or 16 characters
	 * long, part of the name will be truncated at the end to allow the addition of
	 * the two digits.
	 *
	 ...
	 * information from the username.dat file, in the world/player folder.
	 * <p>
	 ...
	 * sleeping or has this flag set, then time will advance to the next day. If
	 * everyone has this flag set but no one is actually in bed, then nothing will
	 * happen.
	 *
	 ...
	 * particular location (as far as the client is concerned). This will not work
	 * without a note block. This will not work with cake.
	 *
	 ...
	 * be heard by the player if their client does not have the respective sound for
	 * the value passed.
	 *
	 * @param location the location to play the sound
	 ...
	 * location. This will not actually change the world in any way.
	 *
	 ...
	 * location. The updated cuboid must be entirely within a single chunk. This
	 * will not actually change the world in any way.
	 * <p>
	 * At least one of the dimensions of the cuboid must be even. The size of the
	 * data buffer must be 2.5*sx*sy*sz and formatted in accordance with the
	 * Packet51 format.
	 *
	 ...
	 * location. This will not actually change the world in any way. This method
	 * will use a sign at the location's block or a faked sign sent via
	 * {@link #sendBlockChange(org.bukkit.Location, int, byte)} or
	 * {@link #sendBlockChange(org.bukkit.Location, org.bukkit.Material, byte)}.
	 * <p>
	 * If the client does not have a sign at the given location it will display an
	 * error message to the user.
	 *
	 * @param loc   the location of the sign
	 * @param lines the new text on the sign or null to clear it
	 * @throws IllegalArgumentException if location is null
	 * @throws IllegalArgumentException if lines is non-null and has a length less
	 *                                  than 4
	 */
	 ...
	 * streaming the map in the normal manner is not desirable.
	 *
	 ...
	 */
    void setPlayerListHeaderFooter(net.md_5.bungee.api.chat.BaseComponent[] header,
	...
	/**
	 * Show the given title to the player, along with the last subtitle set, using
	 * the last set times
	 * 
	 ...
	 */
	@Deprecated
    void showTitle(net.md_5.bungee.api.chat.BaseComponent title, net.md_5.bungee.api.chat.BaseComponent subtitle,
	 ...
	 *
	 * <p>
	 * This method overrides any previous title, use {@link #updateTitle(Title)} to
	 ...
	 * @throws IllegalArgumentException if the statistic requires an additional
	 *                                  parameter
	 */
	 ...
	 * @throws IllegalArgumentException if the given parameter is not valid for the
	 *                                  statistic
	 */
	 ...
	 * player's time will be kept synchronized to its world time with the specified
	 * offset.
	 * <p>
	 ...
	 * the player's time is absolute and will not change its current time unless
	 * done so with setPlayerTime().
	 *
	 ...
	 * of the player is locked until {@link #resetPlayerWeather()} is used.
	 *
	 ...
	 * taken by specifying a negative amount.
	 *
	 ...
	 * not slept in one or their current bed spawn is invalid.
	 *
	...
	/**
	 * Checks to see if a entity has been hidden from this player compared to
	 * shooter
	 *
	 * @param entity Entity to check
	 * @return True if the provided player is not being hidden from this entity
	 */
    boolean canSee(Entity entity);

	/**
	 * Checks to see if this player is currently standing on a block. This
	 * information may not be reliable, as it is a state provided by the client, and
	 ...
	 * @throws IllegalArgumentException If new speed is less than -1 or greater than
	 *                                  1
	 */
	 ...
	 * <p>
	 * The player's client will download the new texture pack asynchronously in the
	 * background, and will automatically switch to it once the download is
	 * complete. If the client has downloaded and cached the same texture pack in
	 * the past, it will perform a quick timestamp check over the network to
	 * determine if the texture pack has changed and needs to be downloaded again.
	 * When this request is sent for the very first time from a given server, the
	 * client will first display a confirmation GUI to the player before proceeding
	 * with the download.
	 * <p>
	 * Notes:
	 * <ul>
	 * <li>Players can disable server textures on their client, in which case this
	 * method will have no affect on them.
	 * <li>There is no concept of resetting texture packs back to default within
	 * Minecraft, so players will have to relog to do so.
	 * </ul>
	 *
	 * @param url The URL from which the client will download the texture pack. The
	 *            string must contain only US-ASCII characters and should be encoded
	 *            as per RFC 1738.
	 ...
	 * <p>
	 * The player's client will download the new resource pack asynchronously in the
	 * background, and will automatically switch to it once the download is
	 * complete. If the client has downloaded and cached the same resource pack in
	 * the past, it will perform a quick timestamp check over the network to
	 * determine if the resource pack has changed and needs to be downloaded again.
	 * When this request is sent for the very first time from a given server, the
	 * client will first display a confirmation GUI to the player before proceeding
	 * with the download.
	 * <p>
	 * Notes:
	 * <ul>
	 * <li>Players can disable server resources on their client, in which case this
	 * method will have no affect on them.
	 * <li>There is no concept of resetting resource packs back to default within
	 * Minecraft, so players will have to relog to do so.
	 * </ul>
	 *
	 * @param url The URL from which the client will download the resource pack. The
	 *            string must contain only US-ASCII characters and should be encoded
	 *            as per RFC 1738.
	 * @throws IllegalArgumentException Thrown if the URL is null.
	 * @throws IllegalArgumentException Thrown if the URL is too long. The length
	 *                                  restriction is an implementation specific
	 *                                  arbitrary value.
	 ...
	 *                                  {@link org.bukkit.scoreboard.ScoreboardManager
	 *                                  scoreboard manager}
	 * @throws IllegalStateException    if this is a player that is not logged yet
	 *                                  or has logged out
	 */
	 ...
	 * contain a new line, only the first line will be sent.
	 *
	 ...
	 *             The string must contain only US-ASCII characters and should be
	 *             encoded as per RFC 1738.
	 * @param hash A 40 character hexadecimal and lowercase SHA-1 digest of the
	 *             resource pack file.
	 * @throws IllegalArgumentException Thrown if the URL is null.
	 * @throws IllegalArgumentException Thrown if the URL is too long. The length
	 *                                  restriction is an implementation specific
	...
	/**
	 * @return the most recent resource pack status received from the player, or
	 *         null if no status has ever been received from this player.
	 */
	org.bukkit.event.player.PlayerResourcePackStatusEvent.Status getResourcePackStatus();

	/**
	 * @return the most recent resource pack hash received from the player, or null
	 *         if no hash has ever been received from this player.
	 */
	String getResourcePackHash();

	/**
	 * @return true if the last resource pack status received from this player was
	 *         {@link org.bukkit.event.player.PlayerResourcePackStatusEvent.Status#SUCCESSFULLY_LOADED}
	 */
    ...
    class Spigot extends Entity.Spigot {
		 ...
		 *
		 * @return whether or not the player affects mob spawning.
		 */
		 ...
		 *
		 * @param affects whether or not the player should affect spawning or not.
		 */
