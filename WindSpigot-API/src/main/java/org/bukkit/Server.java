...
public interface Server extends PluginMessageRecipient {
	...
	/**
	 * Used for all announcement messages, such as informing users that a player has
	 * joined.
	 * <p>
	 * For use in {@link #broadcast(java.lang.String, java.lang.String)}.
	 */
	...
	/**
	 * Gets a view of all currently logged in players. This
	 * {@linkplain Collections#unmodifiableCollection(Collection) view} is a reused
	 * object, making some operations like {@link Collection#size()}
	 * zero-allocation.
	 * <p>
	 * The collection is a view backed by the internal representation, such that,
	 * changes to the internal state of the server will be reflected immediately.
	 * However, the reuse of the returned collection (identity) is not strictly
	 * guaranteed for future or all implementations. Casting the collection, or
	 * relying on interface implementations (like {@link Serializable} or
	 * {@link List}), is deprecated.
	 * <p>
	 * Iteration behavior is undefined outside of self-contained main-thread uses.
	 * Normal and immediate iterator use without consequences that affect the
	 * collection are fully supported. The effects following (non-exhaustive)
	 * {@link Entity#teleport(Location) teleportation},
	 * {@link Player#setHealth(double) death}, and {@link Player#kickPlayer( String)
	 * kicking} are undefined. Any use of this collection from asynchronous threads
	 * is unsafe.
	 * <p>
	 * For safe consequential iteration or mimicking the old array behavior, using
	 * {@link Collection#toArray(Object[])} is recommended. For making snapshots,
	 * {@link ImmutableList#copyOf(Collection)} is recommended.
	 *
	...
	void setMaxPlayers(int maxPlayers); // [Nacho-0021] Add setMaxPlayers within Bukkit.getServer() and
												// SetMaxSlot Command
	...
	/**
	 * Get an ID of this server. The ID is a simple generally alphanumeric ID that
	 * can be used for uniquely identifying this server.
	 *
	...
	/**
	 * Gets the update folder. The update folder is used to safely update plugins at
	 * the right moment on a plugin load.
	 *
	 ...
	 * <b>Example Usage:</b>
	 * <ul>
	 * <li>A value of 1 will mean the server will attempt to spawn monsters every
	 * tick.
	 * <li>A value of 400 will mean the server will attempt to spawn monsters every
	 * 400th tick.
	 ...
	 * </ul>
	 * <p>
	 * <b>Note:</b> If set to 0, monsters spawning will be disabled. We recommend
	 * using spawn-monsters to control this instead.
	 * <p>
	 * Minecraft default: 1.
	 *
	...
	/**
	 * Attempts to match any players with the given name, and returns a list of all
	 * possibly matches.
	 * <p>
	 ...
	 *
	 * <ul>
	 * <li>Vanilla behaviour: check for collisions and move the player if needed.
	 * <li>Exact behaviour: spawn players exactly where they should be.
	 * </ul>
	 *
	 * @return true if exact location locations are used for spawning, false for
	 *         vanilla collision detection or otherwise
	 *
	 ...
	 * <p>
	 * This method may involve a blocking web request to get the UUID for the given
	 * name.
	 * <p>
	 * This will return an object even if the player does not exist. To this method,
	 * all players will exist.
	 *
	 * @deprecated Persistent storage of users should be by UUID as names are no
	 *             longer unique past a single session.
	 ...
	 * <p>
	 * Bans by name are no longer supported and this method will return null when
	 * trying to request them. The replacement is bans by UUID.
	 *
	...
	/**
	 * Gets a {@link ConsoleCommandSender} that may be used as an input source for
	 * this server.
	 *
	 ...
	 * {@link InventoryType#CHEST}, the new inventory has a size of 27; otherwise
	 * the new inventory has the normal size for its type.
	 *
	...
	/**
	 * Creates an empty inventory with the specified type and title. If the type is
	 * {@link InventoryType#CHEST}, the new inventory has a size of 27; otherwise
	 * the new inventory has the normal size for its type.<br>
	 * It should be noted that some inventory types do not support titles and may
	 * not render with said titles on the Minecraft client.
	 *
	 ...
	 * indicates that it is synchronized, but a mismatch <b>does not preclude</b>
	 * the same assumption.
	 *
	 ...
	 *         implementation to indicate no defined icon, but this behavior is not
	 *         guaranteed
	 */
	 ...
	 * server-icon.
	 * <p>
	 * Size and type are implementation defined. An incompatible file is guaranteed
	 * to throw an implementation-defined {@link Exception}.
	 *
	 * @param file the file to load the from
	 * @throws IllegalArgumentException if image is null
	 * @throws Exception                if the image does not meet current server
	 *                                  server-icon specifications
	 * @return a cached server-icon that can be used for a
	 *         {@link ServerListPingEvent#setServerIcon(CachedServerIcon)}
	 */
	...
	/**
	 * Set the idle kick timeout. Any players idle for the specified amount of time
	...
	class Spigot {
		...
		/**
		 * Restart the server. If the server administrator has not configured
		 * restarting, the server will stop.
		 */
		public void restart() {
			throw new UnsupportedOperationException("Not supported yet.");
		}
