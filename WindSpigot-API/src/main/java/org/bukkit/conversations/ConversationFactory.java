 ...
 * a predefined template. A ConversationFactory is typically created when a
 * plugin is instantiated and builds a Conversation each time a user initiates a
 * conversation with the plugin. Each Conversation maintains its own state and
 * calls back as needed into the plugin.
 * <p>
 * The ConversationFactory implements a fluid API, allowing parameters to be set
 * as an extension to the constructor.
 */
public class ConversationFactory {
	...
	/**
	 * Sets the modality of all {@link Conversation}s created by this factory. If a
	 * conversation is modal, all messages directed to the player are suppressed for
	...
	/**
	 * Sets the local echo status for all {@link Conversation}s created by this
	 * factory. If local echo is enabled, any text submitted to a conversation gets
	 * echoed back into the submitter's chat window.
	 *
	...
	/**
	 * Sets the number of inactive seconds to wait before automatically abandoning
	...
	/**
	 * Sets the player input that, when received, will immediately terminate the
	 * conversation.
	 *
	...
	/**
	 * Adds a {@link ConversationAbandonedListener} to all conversations constructed
	 * by this factory.
	 *
	...
	/**
	 * Constructs a {@link Conversation} in accordance with the defaults set for
	 * this factory.
	 *
