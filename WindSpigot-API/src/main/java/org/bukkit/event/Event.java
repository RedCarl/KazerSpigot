...
public abstract class Event {
	 ...
	 * asynchronous.
	 *
	 * @param isAsync true indicates the event will fire asynchronously, false by
	...
	/**
	 * Any custom event that should not by synchronized with other events must use
	 * the specific constructor. These are the caveats of using an asynchronous
	 * event:
	 * <ul>
	 * <li>The event is never fired from inside code triggered by a synchronous
	 * event. Attempting to do so results in an
	 * {@link java.lang.IllegalStateException}.
	 * <li>However, asynchronous event handlers may fire synchronous or asynchronous
	 * events
	 * <li>The event may be fired multiple times simultaneously and in any order.
	 * <li>Any newly registered or unregistered handler is ignored after an event
	 * starts execution.
	 * <li>The handlers for this event may block for any length of time.
	 * <li>Some implementations may selectively declare a specific event use as
	 * asynchronous. This behavior should be clearly defined.
	...
	public enum Result {
		...
		DEFAULT,
		/**
		 * Allow / Force the event. The action indicated by the event will take place if
